...
window.setSimRoute = function(routeKey) {
  simSelectedRoute = routeKey;
  document.querySelectorAll('.sim-route-pill').forEach(btn => {
    btn.classList.toggle('active', btn.getAttribute('data-sim-route') === routeKey);
  });
  window.setSimStep(1);
  if (typeof window.focus3DRoute === 'function') {
    if (routeKey === 'danang') window.focus3DRoute('danang-laos');
    else if (routeKey === 'nhatrang') window.focus3DRoute('nhatrang-boy');
    else if (routeKey === 'hanoi') window.focus3DRoute('hanoi-huunghi');
  }
};

window.setSimStep = function(stepNum) {
  simCurrentStep = stepNum;
  const data = SIMULATION_DATA[simSelectedRoute] || SIMULATION_DATA['danang'];
  const stepInfo = data.steps[stepNum - 1];

  // Update Step Cards
  document.querySelectorAll('.pipeline-step-card').forEach((card, idx) => {
    card.classList.toggle('active', idx + 1 === stepNum);
    card.classList.toggle('completed', idx + 1 < stepNum);
  });
...
window.toggleSimPlay = function() {
  ...
  if (playBtn) {
    playBtn.innerHTML = simIsPlaying ? '⏸ Pause Automation' : '▶ Play Automation';
  }
...
window.simNextStep = function() {
  let next = simCurrentStep + 1;
  if (next > 4) next = 1;
  window.setSimStep(next);
};

window.simPrevStep = function() {
  let prev = simCurrentStep - 1;
  if (prev < 1) prev = 4;
  window.setSimStep(prev);
};

/* ============================================================
   3D TILT CALCULATOR & ESTIMATOR
   ============================================================ */

window.renderRouteOptions3D = function(city) {
  const container = document.getElementById('calc-routes-container-3d');
  if (!container) return;
  container.innerHTML = '';
  ...
  routes.forEach((route, index) => {
    ...
    card.className = 'route-option-card-3d' + (index === selectedRouteIndex ? ' active' : '');
    ...
    card.innerHTML = `
      <div class="roc-radio-3d"></div>
      <div class="roc-info-3d">
        <div class="roc-title-3d">${nameText}</div>
        <div class="roc-desc-3d">${descText}</div>
      </div>
      <div class="roc-price-3d">${route.price.toLocaleString('vi-VN')} ₫</div>
    `;

    card.addEventListener('click', () => {
      selectedRouteIndex = index;
      document.querySelectorAll('.route-option-card-3d').forEach(c => c.classList.remove('active'));
      card.classList.add('active');
      window.calculateTotal3D();
    });
...
window.calculateTotal3D = function() {
  const routes = cityRoutes[selectedCity] || [];
  const route = routes[selectedRouteIndex] || routes[0];
  const routePrice = route ? route.price : 0;

  const evisaSelect = document.getElementById('calc-evisa-speed-3d');
  const multiCheck = document.getElementById('calc-multi-entry-3d');
  ...
  const totalDisplay = document.getElementById('calc-total-display-3d');
  const breakdownDisplay = document.getElementById('calc-breakdown-display-3d');
  ...
  if (breakdownDisplay && route) {
    ...
    if (evisaPrice > 0 && evisaSelect) {
      parts.push(evisaSelect.options[evisaSelect.selectedIndex].text);
    }
    if (multiPrice > 0) {
...
window.handleCalcBooking3D = function(channel) {
  const routes = cityRoutes[selectedCity] || [];
  const route = routes[selectedRouteIndex] || routes[0];
  const totalDisplay = document.getElementById('calc-total-display-3d') ? document.getElementById('calc-total-display-3d').textContent : '';
  const breakdown = document.getElementById('calc-breakdown-display-3d') ? document.getElementById('calc-breakdown-display-3d').textContent : '';
...
` +
    `⚡ *3D AUTOMATION VISA RUN BOOKING*
` +
...
` +
    `• *Legal Terms:* Agreed & Accepted (100% Refund Guarantee)

` +
    `Please confirm instant seat availability. Thank you!`;
...
/* ============================================================
   MULTI-LANGUAGE SWITCHER (EN, VI, RU, AR, KR)
   ============================================================ */

window.switchLanguage3D = function(lang) {
  ...
  if (typeof window.renderRouteOptions3D === 'function') {
    window.renderRouteOptions3D(selectedCity);
  }
  if (typeof window.calculateTotal3D === 'function') {
    window.calculateTotal3D();
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize Calculator City Buttons
  document.querySelectorAll('.calc-city-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      document.querySelectorAll('.calc-city-btn').forEach(b => b.classList.remove('active'));
      this.classList.add('active');
      selectedCity = this.getAttribute('data-city') || 'danang';
      selectedRouteIndex = 0;
      window.renderRouteOptions3D(selectedCity);
      window.calculateTotal3D();
    });
  ...
  const evisaSelect = document.getElementById('calc-evisa-speed-3d');
  const multiCheck = document.getElementById('calc-multi-entry-3d');
  if (evisaSelect) evisaSelect.addEventListener('change', window.calculateTotal3D);
  if (multiCheck) multiCheck.addEventListener('change', window.calculateTotal3D);

  // 3D Parallax Tilt on Card
  const calcCard = document.querySelector('.calc-card-3d');
  if (calcCard) {
    calcCard.addEventListener('mousemove', (e) => {
      const rect = calcCard.getBoundingClientRect();
      const x = e.clientX - rect.left - rect.width / 2;
      const y = e.clientY - rect.top - rect.height / 2;
      const rotateX = -(y / rect.height) * 12;
      const rotateY = (x / rect.width) * 12;
      calcCard.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateY(-4px)`;
    });
    calcCard.addEventListener('mouseleave', () => {
      calcCard.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg) translateY(0)';
    });
  }

  // Border Guide Tabs
  document.querySelectorAll('.guide-tab-3d').forEach(btn => {
    btn.addEventListener('click', function() {
      document.querySelectorAll('.guide-tab-3d').forEach(b => b.classList.remove('active'));
      document.querySelectorAll('.guide-pane-3d').forEach(p => p.classList.remove('active'));
      this.classList.add('active');
      const paneId = this.getAttribute('data-guide');
  ...
  // Mobile Menu
  const hamburger = document.getElementById('hamburger-3d');
  const navLinks = document.getElementById('nav-links-3d');
  ...
  // Scroll to top
  const topBtn = document.getElementById('scroll-top-3d');
  if (topBtn) {
  ...
  // Load Initial Language & Defaults
  let initLang = 'en';
  try {
  ...
  window.switchLanguage3D(initLang);
  window.setSimStep(1);
  window.renderRouteOptions3D(selectedCity);
  window.calculateTotal3D();
});
